...
type rSASecurity struct {
	...
	pubModTime time.Time       //isFileInit＝＝true有效, 公钥文件最后的修改时间
	priModTime time.Time       //isFileInit＝＝true有效, 私钥文件最后的修改时间
}

func (this *rSASecurity) String(in string, mode int) (string, error) {
	var inByte []byte
	var err error
	if mode == MODE_PRIKEY_ENCRYPT {
		inByte = []byte(in)
	} else if mode == MODE_PUBKEY_DECRYPT {
...
func NewRSASecurityByFile(pubFile, priFile string, ifCache bool) (rsa *rSASecurity, pubKeyErr, priKeyErr error) {
	rsa = &rSASecurity{}
	pubKeyErr, priKeyErr = rsa.InitByFile(pubFile, priFile)
	return
}
*/

func (this *rSASecurity) InitByFile(pubFile, priFile string) (pubkeyErr, prikeyErr error) {
...
func (this *rSASecurity) InitPRKByFile(priFile string) (prikeyErr error) {
	this.isFileInit = true
	//this.pubStr = pubFile
	this.priStr = priFile
